...
function getPalette(stylesArtboard) {
    ...
    const findPalette = stylesArtboard.filter(item => {
        return item.name === "palette";
    })[0].children;

    const paletteArtboard = findPalette.filter(item => {
        return item.name === "colors";
    })[0].children;

    // console.log(paletteArtboard);
        ...
        if (fontItem.children) {
            ...
            let fontObj = {
                [fontItem.name]: {
                    ...
                    size: {
                        value: `${fontItem.style.fontSize}px`,
                        type: "typography"
                    },
                    weight: {
...
getJSONBtn.onclick = function () {
    ...
    let personalToken = document.getElementById('token').value;
    let fileID = document.getElementById('file-id').value;
    getStylesArtboard(
        personalToken,
        fileID
    );
